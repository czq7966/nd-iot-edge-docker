...
docker run -d -p 80:8080      --network nd-iot-edge --name nd-iot-edge --privileged -v /dev/ttyUSB0:/dev/ttyUSB0 -e IOT_CODE_BRANCH=dev -e IOT_ENABLE_AUTO_UPDATE=0  -e IOT_ENABLE_NGINX=1  -e IOT_ENABLE_MQTT=1 -e IOT_ENABLE_EDG=1 -e IOT_APP_ID=ndiot -e IOT_EDG_ID=ndiot-edg-test nd-iot-edge:22
